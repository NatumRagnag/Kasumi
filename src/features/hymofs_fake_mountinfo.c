...
bool hymo_fake_mi_is_internal_read(void)
{
...
static int regenerate_cache_locked(void)
{
    ...
    if (IS_ERR(f)) {
        ret = PTR_ERR(f);
        hymo_log("fake_mi: filp_open(/proc/self/mountinfo) failed pid=%d comm=%s ret=%d\n",
    ...
    while (total < FAKE_MI_SCRATCH) {
        ...
        if (r < 0) {
            ret = r;
            hymo_log("fake_mi: kernel_read(/proc/self/mountinfo) failed pid=%d comm=%s ret=%zd pos=%lld total=%zu\n",
    ...
    new_buf = NULL;
    ret = 0;
    hymo_log("fake_mi: regenerated pid=%d comm=%s raw_len=%zu fake_len=%zu gen=%llu\n",
...
void hymo_fake_mi_invalidate_all(void)
{
    unsigned long flags;
    int i;
...
int hymo_fake_mi_prepare(bool force)
{
    int ret = 0;
...
ssize_t hymo_fake_mi_serve(struct file *file, void __user *userbuf,
...
ssize_t hymo_fake_mi_read_iter(struct kiocb *iocb, struct iov_iter *to)
{
    struct file *file;
    size_t avail;
    size_t to_copy;
...
int hymo_fake_mi_lookup_mount_id(const char *path)
{
    size_t path_len;
    size_t in = 0;
    int ret = -ENOENT;
...
int hymo_fake_mi_init(void)
{
    mutex_init(&g_cache.lock);
    memset(g_cursors, 0, sizeof(g_cursors));

    ptr_filp_open  = (void *)hymofs_lookup_name("filp_open");
    ptr_filp_close = (void *)hymofs_lookup_name("filp_close");
    ptr_kernel_read = (void *)hymofs_lookup_name("kernel_read");
    ptr_override_creds = (void *)hymofs_lookup_name("override_creds");
    ptr_revert_creds = (void *)hymofs_lookup_name("revert_creds");
    fake_mi_init_task_ptr = (struct task_struct *)hymofs_lookup_name("init_task");
    fake_mi_ksu_cred_pp = (struct cred **)hymofs_lookup_name("ksu_cred");

    if (!ptr_filp_open || !ptr_filp_close || !ptr_kernel_read) {
        pr_warn("HymoFS fake_mi: symbol resolution failed (filp_open=%p filp_close=%p kernel_read=%p); feature disabled\n",
    ...
    pr_info("HymoFS fake_mi: initialized (ksu_cred=%p fallback_kcred=%p)\n",
            fake_mi_ksu_cred_pp ? READ_ONCE(*fake_mi_ksu_cred_pp) : NULL,
            fake_mi_kcred);
    return 0;
}

void hymo_fake_mi_exit(void)
{
    mutex_lock(&g_cache.lock);
    if (g_cache.buf) {
        vfree(g_cache.buf);
